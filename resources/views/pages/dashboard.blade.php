                                                ...
                                                <div class="tab-pane fade" id="tranchepay" role="tabpanel">
                                                    ...
                                                    <div class="table-responsive">
                                                        <table class="table">
                                                            <thead>
                                                                <tr>
                                                                    <th>Tranchepay Product</th>
                                                                    <th>Plan / Monthly Pay</th>
                                                                    <th>Status</th>
                                                                    <th>Next Payment</th>
                                                                    <th>Action</th>
                                                                </tr>
                                                            ...
                                                            <tbody>
                                                                ...
                                                                @foreach($my_tranchepays as $tranchepay)
                                                                        ...
                                                                        @php
                                                                            //GET PLAN DETAILS
                                                                              ...
                                                                              curl_setopt_array($curl, array(
                                                                                CURLOPT_URL => "https://api.paystack.co/plan/$tranchepay->plan_code",
                                                                              ...
                                                                              $err = curl_error($curl);
                                                                                ...
                                                                                $payment_amount = $json->data->amount ?? null;
                                                                                $payments_count = $json->data->payments_count ?? null;
                                                                                //$interval = $json->data->plan[0]->interval ?? null;
                                                                                //$payments_count = $json->data->payments_count;
                                                                                //$next_payment_date = $json->data->next_payment_date ?? null;
                                                                                ...
                                                                                if(isset($json->data->subscriptions[0]->next_payment_date)){
                                                                                            ...
                                                                                            $color = "green";
                                                                                            $payment_status = "";
                                                                                            $amount_status = "₦".number_format($payment_amount/100, 2) ?? "";
                                                                                
                                                                                            ////////////////////  GET SUBSCRIPTION DATA  /////////////////////
   
                                                                                            $subscription_code = $json->data->subscriptions[0]->subscription_code ?? null;
                                                                                        
                                                                                            curl_setopt_array($curl, array(
                                                                                            CURLOPT_URL => "https://api.paystack.co/subscription/$subscription_code",
                                                                                            CURLOPT_RETURNTRANSFER => true,
                                                                                            CURLOPT_ENCODING => "",
                                                                                            CURLOPT_MAXREDIRS => 10,
                                                                                            CURLOPT_TIMEOUT => 30,
                                                                                         ...
                                                                                         //check if paystack request was successful
                                                                                          if($json->status == true){
                                                                                                ...
                                                                                                $message = $json->message;//MESSAGE
                                                                                                $payments_count = $json->data->payments_count ?? null;
                                                                                                $interval = $json->data->plan->interval ?? null;
                                                                                                $next_payment_date = $json->data->next_payment_dated ?? null;
                                                                                          }
                                                                                          
                                                                                            $next_payment = strtotime($next_payment); //--> which results to 1332866820
                                                                                            $next_payment_time = date('H:i:s', $next_payment);
                                                                                            $next_payment_date = date('d-m-Y', $next_payment);
                                                                                            
                                                                                            if($payments_count == 0){$payment_status = $payments_count." No payment made yet";$status = "Active";$color = "blue";}
                                                                                            if($payments_count == 1){$payment_status = $payments_count." month paid";}
                                                                                            if($payments_count > 1){$payment_status = $payments_count." months paid";}
                                                                                            if($payments_count == null){$payment_status = "";}
                                                                    ...
                                                                    <tr>
                                                                        <td>{{ $tranchepay->subscription_name }}</td>
                                                                        <td>{{ $tranchepay->payment_interval }} payment for {{ $tranchepay->charge_limit }} months</td>
                                                                        <td><i><div style=color:{{ $color }};><b>{{ $status }}</b> <br><small>{{ $payment_status ?? ""}}</small> </div></i></td>
                                                                        <td><i>{{ $next_payment_date ?? 'Not Set' }}<i> <br> <i>Time: {{ $next_payment_time ?? 'Not Set' }}<i> <br> <small><b>{{ $amount_status ?? ""}}</b></small></td>
                                                                        
                                                                        @if($flagx == true)
                                                                        <td><a href="/generate/subscription/{{ $tranchepay->plan_code }}/update/link"><b>Change Payment Card / Cancel Installment</b></a></td>
                                                                        @endif
                                                                        
                                                                        @if($flagx == false)
                                                                        <td><a href="/paystack/pay/subscription/{{ $tranchepay->plan_code }}/{{ $tranchepay->pid_subscription }}/payment/link"><b><div style="color:green;">Start Subscription</div></b></a></td>
                                                                        @endif
